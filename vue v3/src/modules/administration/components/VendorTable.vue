...
const getItems = async () => {
    try {
        ...
        const res = await vendorStore.search(payload, params);
...
const updateStatus = async () => {
    try {
        ...
        await vendorStore.updateStatus(selectedItem.value.id, payload);
        await getItems();
    } finally {
        loading.value = false;
    }
...
const save = async () => {
    ...
    try {
        busy.value = true;
        if (isEditMode.value) {
            await vendorStore.update(selectedItem.value.id, modifiedFormData);
            closeDialog();
            await getItems();
        } else {
            await vendorStore.create(modifiedFormData);
            closeDialog();
            await getItems();
        }
    } catch (error) {
...
const deleteItem = async () => {
    try {
        ...
        if (selectedItem.value) {
            await vendorStore.deleteItem(selectedItem.value.id);
        }
        await getItems();
    } finally {
        loading.value = false;
...
<template>
    ...
    >
        <template #header>
            <div class="flex justify-content-between mb-2">
                ...
                <Button
                    ...
                    v-if="$ability.can('create vendors')"
                />
            </div>
        </template>
        <template #empty>
            <span :data-testid="'empty-data-table'">
                {{
                    $t('common.datatable_not_found', {
                        item: $t('common.vendors').toLowerCase()
                    })
                }}
            </span>
        </template>
        ...
        <Column
            v-if="
                $ability.can('update vendors') || $ability.can('delete vendors')
            "
        >
            <template #body="{ data, index }">
                <Button
    ...
    <Dialog
        ...
        modal
        :closable="false"
        v-if="$ability.can('create vendors') || $ability.can('update vendors')"
    >
        <template #header>
    ...
    <Confirmation
        ...
        v-if="$ability.can('update vendors')"
    />

    <Confirmation
        v-model="deleteDialog"
        show-alert-icon
        :header="t('common.delete_item_header', { item: t('common.vendor') })"
        ...
        v-if="$ability.can('delete vendors')"
    />
    ...
    >
        ...
        <template #default>
            <AuditTable entity="vendor" :entity_id="selectedItem?.id" />
        </template>
    </Dialog>
</template>
