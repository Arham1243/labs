...
import { useClientSectorStore } from '@/modules/administration/stores';
import { useGlobalStore } from '@/stores';
import { useHelpers } from '@/composables';
import { PaginationOptions, SortFilterOptions } from '@/config';
import AuditTable from '@/components/common/AuditTable.vue';

const clientSectorStore = useClientSectorStore();
    ...
    const allMenuItems = [
        {
            ...
            permission: 'update client sectors'
        },
        {
        ...
        },
        {
            ...
            permission: 'delete client sectors'
        },
        {
            label: t('common.audit_log'),
            icon: 'pi pi-history',
            command: () => showAuditTableDialog(),
            permission: 'view client sectors'
        }
    ];
...
const getItems = async () => {
    try {
        ...
        const res = await clientSectorStore.search(payload, params);
...
const updateStatus = async () => {
    try {
        ...
        await clientSectorStore.updateStatus(selectedItem.value.id, payload);
        await getItems();
    } finally {
        loading.value = false;
    }
...
const save = async () => {
    ...
    try {
        busy.value = true;
        if (isEditMode.value) {
            await clientSectorStore.update(
            ...
            await clientSectorStore.create(modifiedFormData);
            closeDialog();
            await getItems();
        }
    } catch (error) {
...
const deleteItem = async () => {
    try {
        loading.value = true;
        await clientSectorStore.deleteItem(selectedItem.value.id);
        await getItems();
    } finally {
        loading.value = false;
    }
...
<template>
    ...
    >
                ...
                <Button
                    :label="
                        $t('common.create_item_btn_label', {
                            item: $t('client_sector.sector')
                        })
                    "
                    icon="pi pi-plus"
                    @click="openDialog('add')"
                    data-testId="add-new-client-sector-button"
                    v-if="$ability.can('create client sectors')"
                />
            </div>
        </template>
        <template #empty>
            <span :data-testid="'empty-data-table'">
                {{
                    $t('common.datatable_not_found', {
                        item: $t('client_sector.client_sectors').toLowerCase()
                    })
                }}
            </span>
        </template>
        ...
        <Column
            v-if="
                $ability.can('update client sectors') ||
                $ability.can('delete client sectors')
            "
        >
            <template #body="{ data, index }">
                <Button
    ...
    <Confirmation
        ...
        v-if="$ability.can('update client sectors')"
    />
    ...
    <Confirmation
        ...
        show-alert-icon
        :header="
            t('common.delete_item_header', { item: t('client_sector.title') })
        "
        ...
        v-if="$ability.can('delete client sectors')"
    />
    ...
    <Dialog
        ...
        :closable="false"
        v-if="
            $ability.can('create client sectors') ||
            $ability.can('update client sectors')
        "
    >
        <template #header>
        ...
        <div class="p-fluid formgrid grid">
            <div class="field col-12 mb-4">
                <label
                    data-testid="sector-name-label"
                    class="block required mb-2"
                    for="name"
                    >{{ $t('common.sector_name') }}</label
                >
                <InputField
                    data-testid="sector-name-input"
                    :placeholder="$t('common.sector_name')"
                    id="name"
                    type="text"
                    variant="text"
    ...
    >
        ...
        <template #default>
            <AuditTable entity="client-sector" :entity_id="selectedItem?.id" />
        </template>
    </Dialog>
</template>
