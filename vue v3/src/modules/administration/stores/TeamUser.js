...
import { TeamUserService } from '@/modules/administration/services';
import { useI18n } from 'vue-i18n';

export const useTeamUserStore = defineStore('TeamUserStore', () => {
    const globalStore = useGlobalStore();
    const { t } = useI18n();

    const searchUsers = (id, action, payload, params) => {
        return globalStore.actionWrapper(async () => {
            const res = await TeamUserService.searchUsers(
                id,
                action,
                payload,
                params
    ...
    const syncUsers = (id, action, payload) => {
        return globalStore.actionWrapper(async () => {
            const res = await TeamUserService.syncUsers(id, action, payload);
