...
import Table from '@/modules/administration/components/policy/tables/ContactSourceTable.vue';
import { searchContactSourceMock } from '@/modules/administration/tests/mocks/ContactSource.service.mocks';
import { updateAbility } from '@/plugins/ability';
import { ContactSourceService } from '@/modules/administration/services';

vi.spyOn(ContactSourceService, 'create').mockRejectedValue({
    response: {
        status: 422,
        data: {
            errors: {
...
describe('Contact Source Dialog Form', () => {
    beforeEach(() => {
        searchContactSourceMock();
        updateAbility(['create contact sources', 'update contact sources']);
    });
    ...
    it('renders the form with correct labels, input fields, and buttons', async () => {
        ...
        expect(
            getDialogByTestId('contact-source-label').textContent.trim()
        ).toBe('Contact Source Name');
        expect(getDialogByTestId('status-label').textContent.trim()).toBe(
            'Status'
        );

        expect(getDialogByTestId('contact-source-input')).toBeTruthy();
    ...
    it('ensures input fields have the correct types', async () => {
        ...
        await fireEvent.click(getByTestId('create-new-button'));
        expect(getByTestId('contact-source-input')).to.have.property(
            'type',
            'text'
        );
    ...
    it('displays validation errors when required fields are missing', async () => {
        ...
        expect(ContactSourceService.create).toHaveBeenCalled();
    });

    it('closes the dialog after successfully submitting the form', async () => {
        const { getByTestId, queryByTestId } = render(Table);

        vi.spyOn(ContactSourceService, 'create').mockResolvedValue({});

        await fireEvent.click(getByTestId('create-new-button'));

        const dialog = getByTestId('dialog');
        expect(dialog).toBeTruthy();
        const nameInput = within(dialog).getByTestId('contact-source-input');
        const saveButton = within(dialog).getByTestId('save-button');
        ...
        expect(ContactSourceService.create).toHaveBeenCalledWith({
            name: 'Unit Test',
            status: 'active'
        });
    });
