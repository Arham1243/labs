...
import TableView from '@/modules/administration/components/policy/tables/ContactSourceTable.vue';
import { searchContactSourceMock } from '@/modules/administration/tests/mocks/ContactSource.service.mocks';
import { updateAbility } from '@/plugins/ability';

describe('ContactSourceTable', () => {
    beforeEach(() => {
        searchContactSourceMock();
        updateAbility(['create contact sources']);
    });
    ...
    it('displays empty state when no data is available', async () => {
        ...
        expect(wrapper.findByTestId('empty-data-table').text()).toBe(
            'No contact sources found.'
        );
    });

    it('renders a new contact source after adding to state', async () => {
        updateAbility(['update contact sources']);
        const wrapper = mount(TableView);
