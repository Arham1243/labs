...
import TableView from '@/modules/administration/components/policy/tables/ApplicantTypeTable.vue';
import { searchApplicantTypeMock } from '@/modules/administration/tests/mocks/ApplicantType.service.mocks';
import { updateAbility } from '@/plugins/ability';

describe('ApplicantTypeTable', () => {
    beforeEach(() => {
        searchApplicantTypeMock();
        updateAbility(['create applicant types']);
    });
    ...
    it('displays empty state when no data is available', async () => {
        ...
        expect(wrapper.findByTestId('empty-data-table').text()).toBe(
            'No applicant types found.'
        );
    });

    it('renders a new applicant type after adding to state', async () => {
        updateAbility(['update applicant types']);
        const wrapper = mount(TableView);
