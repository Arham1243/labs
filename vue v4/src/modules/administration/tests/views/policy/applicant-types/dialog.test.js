...
import Table from '@/modules/administration/components/policy/tables/ApplicantTypeTable.vue';
import { searchApplicantTypeMock } from '@/modules/administration/tests/mocks/ApplicantType.service.mocks';
import { updateAbility } from '@/plugins/ability';
import { ApplicantTypeService } from '@/modules/administration/services';

vi.spyOn(ApplicantTypeService, 'create').mockRejectedValue({
    response: {
        status: 422,
        data: {
            errors: {
...
describe('Applicant Type Dialog Form', () => {
    beforeEach(() => {
        searchApplicantTypeMock();
        updateAbility(['create applicant types', 'update applicant types']);
    });
    ...
    it('renders the form with correct labels, input fields, and buttons', async () => {
        ...
        expect(
            getDialogByTestId('applicant-type-label').textContent.trim()
        ).toBe('Applicant Type Name');
        expect(getDialogByTestId('status-label').textContent.trim()).toBe(
            'Status'
        );

        expect(getDialogByTestId('applicant-type-input')).toBeTruthy();
    ...
    it('ensures input fields have the correct types', async () => {
        ...
        await fireEvent.click(getByTestId('create-new-button'));
        expect(getByTestId('applicant-type-input')).to.have.property(
            'type',
            'text'
        );
    ...
    it('displays validation errors when required fields are missing', async () => {
        ...
        expect(ApplicantTypeService.create).toHaveBeenCalled();
    });

    it('closes the dialog after successfully submitting the form', async () => {
        const { getByTestId, queryByTestId } = render(Table);

        vi.spyOn(ApplicantTypeService, 'create').mockResolvedValue({});

        await fireEvent.click(getByTestId('create-new-button'));

        const dialog = getByTestId('dialog');
        expect(dialog).toBeTruthy();
        const nameInput = within(dialog).getByTestId('applicant-type-input');
        const saveButton = within(dialog).getByTestId('save-button');
        ...
        expect(ApplicantTypeService.create).toHaveBeenCalledWith({
            name: 'Unit Test',
            status: 'active'
        });
    });
