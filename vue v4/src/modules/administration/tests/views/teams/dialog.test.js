...
import Table from '@/modules/administration/components/teams/tables/TeamTable.vue';
...
vi.spyOn(TeamService, 'create').mockRejectedValue({
    response: {
        status: 422,
        data: {
            errors: {
...
describe('Team Dialog Form', () => {
    beforeEach(() => {
        searchMock();
        updateAbility(['create teams', 'update teams']);
    });

    it('opens the dialog when clicking the "Create New" button', async () => {
        const { getByTestId } = render(Table);

        const button = getByTestId('add-new-team-button');
    ...
    it('closes the dialog when clicking the "Cancel" button', async () => {
        ...
        await fireEvent.click(getByTestId('add-new-team-button'));
