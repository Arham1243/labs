...
import { useContactSourceStore } from '@/modules/administration/stores';
import { useGlobalStore } from '@/stores';
import { useHelpers } from '@/composables';
import { PaginationOptions, SortFilterOptions } from '@/config';
import AuditTable from '@/components/common/AuditTable.vue';

const contactSourceStore = useContactSourceStore();
...
const getItems = async () => {
    try {
        ...
        const res = await contactSourceStore.search(payload, params);
...
const updateStatus = async () => {
    try {
        ...
        await contactSourceStore.updateStatus(selectedItem.value.id, payload);
        await getItems();
    } finally {
        loading.value = false;
    }
...
const save = async () => {
    ...
    try {
        busy.value = true;
        if (isEditMode.value) {
            await contactSourceStore.update(
            ...
            await contactSourceStore.create(modifiedFormData);
            closeDialog();
            await getItems();
        }
    } catch (error) {
...
const deleteItem = async () => {
    try {
        loading.value = true;
        await contactSourceStore.deleteItem(selectedItem.value.id);
        await getItems();
    } finally {
        loading.value = false;
    }
...
<template>
    ...
    >
        <template #header>
            <div class="flex justify-between items-center mb-2">
                ...
                <div>
                    <Button
                        :label="$t('contact_sources.create_btn_label')"
                        icon="pi pi-plus"
            ...
            <span :data-testid="'empty-data-table'">
                {{
                    $t('common.datatable_not_found', {
                        item: $t('contact_sources.title').toLowerCase()
                    })
                }}
            </span>
        </template>
        ...
        <div class="grid grid-cols-12">
            <div class="field col-span-12 mb-6">
                <label
                    ...
                    for="name"
                    >{{ $t('contact_sources.contact_source_name') }}</label
                >
                <InputField
    ...
    >
        ...
        <template #default>
            <AuditTable entity="contact-source" :entity_id="selectedItem?.id" />
        </template>
    </Dialog>
</template>
