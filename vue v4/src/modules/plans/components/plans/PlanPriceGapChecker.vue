...
const month_map = {
    ...Object.fromEntries(months.map((month, index) => [month, index])),
    ...Object.fromEntries(fullMonths.map((month, index) => [month, index]))
};
...
const handleCreatePrice = (gap, event) => {
    ...
    const startDate = parseDisplayDate(gap.start_date);
    const endDate = gap.end_date ? parseDisplayDate(gap.end_date) : null;

    emits('create-price', {
        start_date: formatFormDate(startDate),
        end_date: endDate ? formatFormDate(endDate) : null
    });
};
...
<template>
    <div v-if="priceGaps.length > 0" class="mt-6 mb-2">
        <InputField
            variant="message"
            v-for="(gap, index) in priceGaps" :key="index"
            severity="warn"
        ...
        >
            ...
            <strong class="ml-2">
                {{ getDisplayEndDate(gap.end_date, gap.is_open_ended) === null ? 'no end date' : gap.end_date }}
            </strong
            >. Please
            <a
                href="#"
