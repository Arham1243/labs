...
const getItems = async () => {
    try {
        ...
        const res = await codeSetStore.searchCodeSets(payload, params);
...
const updateStatus = async () => {
    try {
        ...
        await codeSetStore.updateCodeSet(selectedItem.value.id, payload);
        await getItems();
    } finally {
        loading.value = false;
    }
...
const deleteItem = async () => {
    try {
        loading.value = true;
        await codeSetStore.deleteCodeSet(selectedItem.value.id);
        await getItems();
    } finally {
        loading.value = false;
    }
