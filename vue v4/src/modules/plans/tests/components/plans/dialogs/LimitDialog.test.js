        ...
        wrapper = mount(LimitDialog, {
            ...
            global: {
                stubs: {
                    ...
                    InputField: {
                        template: `<div :data-testid="variant === 'switch' ? 'input-switch' : 'input-field'"></div>`,
                        props: [
                            'id',
                            'variant',
                            'addonAfter',
...
describe('LimitDialog - Interaction Tests', () => {
            ...
            global: {
                stubs: {
                    ...
                    InputField: {
                        template: `
                            <input 
                                data-testid="input-switch"                                v-if="variant === 'switch'"
                                type="checkbox"
                                :checked="modelValue" 
                                @change="$emit('update:modelValue', $event.target.checked)" 
                            />
                            <input 
                                data-testid="input-field"                                v-else
                                :value="modelValue" 
    ...
    it('toggles max amount input field when max amount switch is toggled', async () => {
        ...
        expect(wrapper.vm.isMax).toBe(true);
        expect(
            wrapper.findAll('[data-testid="input-field"]').length
        ).toBe(2);
    ...
    it('updates coverage value when input changes', async () => {
        const coverageField = wrapper.findAll(
    ...
    it('updates max amount value when input changes', async () => {
        const maxField = wrapper.findAll(
            '[data-testid="input-field"]'
        )[1];
