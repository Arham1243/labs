...
let special_comp = [
    ...
    'ApiDropdown',
    'Calendar',
    'DatePicker', //Added for PrimeVue 4
    'Checkbox',
    'FileUpload',
    'MultiSelect'
].indexOf(props.field.type);

// Handle both Calendar and DatePicker - convert Calendar to DatePicker automatically
const isDateField = computed(() => {
    return props.field.type === 'Calendar' || props.field.type === 'DatePicker';
});

// DatePicker value conversion
const datePickerValue = computed({
    get() {
        if (isDateField.value && vModel.value instanceof Date) {
            return vModel.value.toISOString();
        }
        return vModel.value;
    },
    set(newValue) {
        if (isDateField.value) {
            if (typeof newValue === 'string' && newValue) {
                try {
                    vModel.value = new Date(newValue);
                } catch (error) {
...
<template>
    ...
    <div
        ...
        :class="
            ...
            (field.class?.includes('flex-row') ? '' : ' flex-col')
        "
    >
        <div v-if="field.additional" class="flex justify-between items-center">
            <label
                for="template"
        ...
        <!-- Handle BOTH Calendar and DatePicker as DatePicker in PrimeVue 4 -->
        <span
            v-if="isDateField"
        ...
        >
            <!-- Use RawField.vue custom field for follow the whole system standards -->
            <InputField
                variant="date"
                v-bind="field.props"
        ...
        <FileUpload
            ...
            :pt="{
                buttonbar: {
                    class: 'p-12 bg-white rounded-border',
                    style: {
        ...
        >
            <template #header="{ chooseCallback }">
                <a
                    class="flex gap-2 items-center text-muted-color"
