        ...
        <Column style="width: 8rem">
            <template #header>
                <div class="flex -ml-2">
                    ...
                    <Button type="button" icon="pi pi-ellipsis-v" class="ml-12 p-button-rounded p-button-text p-button-plain" @click="menuRef.toggle($event)"></Button>
                    <Menu ref="menuRef" popup :model="menuItems" styleClass="w-32"></Menu>
                </div>
                <div></div>
            </template>
            <template #body="{ data }">
                ...
                <td v-if="!data.trash && !data.spam" :style="{ width: '4rem' }">
                    <span class="cursor-pointer ml-12" @click="changeMailType('important', data)">
                ...
                <div :style="{ minWidth: '10rem' }">
                    <div class="flex justify-end w-full px-0">
                        ...
                        <div ref="actionRef" class="action-buttons" style="display: none">
                            <Button @click="onSingleMailAction('archive', data)" type="button" tooltip="Archive" icon="pi pi-inbox" class="h-8 w-8/12 mr-2" v-tooltip.top="'Archive'"></Button>
                            <Button @click="onReplyMail(data)" type="button" icon="pi pi-reply" class="p-button-secondary h-8 w-8/12 mr-2" v-tooltip.top="'Reply'"></Button>
                            <Button @click="onSingleMailAction('trash', data)" type="button" v-tooltip.top="'Trash'" icon="pi pi-trash" class="p-button-danger h-8 w-8/12"></Button>
                        </div>
                    </div>
                </div>
            </template>
