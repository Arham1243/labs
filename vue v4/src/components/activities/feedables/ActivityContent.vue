...
import {
    createActivityConfirmationDialog,
    createActivityDropdownOptions
...
const emit = defineEmits(['delete:feed']);
...
const activitiesDropdownOptions = createActivityDropdownOptions(t);
const primaryEntity = props.feed.entities.find((e) => e.is_primary);

const confirmDialogContent = createActivityConfirmationDialog(t);

const getConfirmActivityDialogContent = () => {
    const title = props.feed.title;
    return confirmDialogContent['delete']?.(title);
};

const { currentModule, currentModuleId, removeActivity } = useActivityStore();
const { mutate: removeActivityMutation } = removeActivity();

const { mutateFeed } = useFeedStore();
const { mutate: feedMutation } = mutateFeed();

const toggle = (event) => {
    menu.value.toggle(event);
};

const getMenuDropdownOptions = () => {
    const type = ref();
    if (['phone', 'chat', 'email', 'text'].includes(props.feed.feedable_type))
        type.value = 'communication';
    else type.value = props.feed.feedable_type;

    const options = activitiesDropdownOptions[type.value]?.() || [];

    return options.map((option) => {
        if (option.label === t('buttons.edit')) {
            return {
                ...option,
                command: () => openDialog(type.value)
            };
        }

        if (option.label === t('buttons.delete')) {
            return {
                ...option,
                command: () => {
                    const confirmActivityDialogContent =
                        getConfirmActivityDialogContent();
                    confirmActivityDialogContent.accept = () => {
                        const data = {
                            ...
                            payload: {
                                ...
                                source: props.feed.feedable_type
                            },
                            activityId: props.feed.feedable_id
                        };

                        removeActivityMutation(data);
                        emit('delete:feed');
                    };

                    confirm.require(confirmActivityDialogContent);
                }
            };
        }
...
const pinFeed = () => {
    const data = {
        ...
        payload: {
            is_pinned: !localFeed.value.is_pinned,
            module: currentModule.value,
            moduleId: currentModuleId.value
        }
    };

    localFeed.value.is_pinned = !localFeed.value.is_pinned;
    feedMutation(data);
};

const onFeedUpdated = (updatedFeed) => {
    console.log(updatedFeed.value);
    // localFeed.value.title = updatedFeed.title;
    // localFeed.value.body = updatedFeed.content;
};

// Sync props to localFeed
watch(
    () => props.feed,
    (newFeed) => {
        localFeed.value = { ...newFeed };
    },
...
<template>
    <div>
        <div
            class="flex justify-between border-b border-gray-300 pb-5 pt-3"
        >
            <div class="flex gap-2 items-center">
                <div class="rounded-full p-2 text-white text-sm" style="background: #14377d">
                    DH
                </div>
                <div>
                    <h6>{Employee Name}</h6>
                    <div class="text-xs mt-1">{Department Name}</div>
                </div>
            </div>
            <div class="text-xs flex flex-col gap-2">
                <div>
                    Date Created:
                    {{ helpers.formatDate(feed.created_at, 'DD-MMM-YYYY') }}
                </div>
                <div>
                    Last Modified:
                    {{ helpers.formatDate(feed.updated_at, 'DD-MMM-YYYY') }}
                </div>
            </div>
        </div>
        ...
        <div class="mt-5">
            <div class="flex justify-between items-center">
                ...
                <div class="flex gap-3 px-1 justify-end">
                    <svg
                        @click="pinFeed"
                    ...
                    <i
                        class="pi pi-ellipsis-v cursor-pointer text-lg p-1"
                        @click="toggle"
                    />
                    <Menu
                        class="w-72"
            ...
            <div class="mt-12 text-sm">
                <p v-html="localFeed?.body.replace(/&amp;nbsp;/g, ' ')"></p>
            </div>
            ...
            <div class="mt-5">
                <div
                    class="shadow-sm flex items-center gap-3 w-8/12 bg-sky-50 rounded-xl p-5"
                >
                    <div class="rounded-full p-2 text-white text-sm" style="background: #14377d">
                        NS
                    </div>
                    <div>
                        <div class="text-sm">
                            @ {Examiner Name} suscipit habitant etiam vitae sed
                            dictum...
                        </div>
